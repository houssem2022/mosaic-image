...
def main(img,input_images): 
  ...
  # target image 
  frame, i= img
  cv2.imwrite("mosaic_pictures\frame%s.jpg" %(i,), frame)
  target_image = Image.open("mosaic_pictures\frame%s.jpg" %(i,)) 
  # size of grid 
  #grid_size = (49, 87)
  grid_size =(31, 56)
  ...
  # output 
  output_filename = "mosaic_pictures\mosaic%s.jpg" %(i,)
  ...
  im3 = Image.open("mosaic_pictures\frame%s.jpg" %(i,)).resize(mosaic_image.size)
  ...
  im2 = cv2.imread("mosaic_pictures\frame%s.jpg" %(i,))
  mask_blur = cv2.GaussianBlur(im2, (3, 3), 0)
  edges = cv2.Canny(mask_blur, 21,21)
  cv2.imwrite("mosaic_pictures\frame%s.jpg" %(i,), edges)

  im21 = Image.open(output_filename)
  edges2=Image.open("mosaic_pictures\frame%s.jpg" %(i,)).resize(im21.size)
...
if __name__ == '__main__': 
  ...
  Parallel(n_jobs=len(total_images))(delayed(opt_code)(img,input_images1) for img in  total_images)
  """  img1 = Image.open('mosaic%s.jpg' %(img[1],) ).resize((1000,600))
    img1.save('mosaic%s.jpg' %(img[1],))
    img2=cv2.imread('mosaic%s.jpg' %(img[1],))
    out.write(img2)"""
